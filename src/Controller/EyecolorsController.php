...
class EyecolorsController extends AppController
{
    ...
    public function view($id = null)
    {
        ...
        $labo = 'http://laborats.weebly.com/' . h(preg_replace('/\s+/', '-', str_replace('é', 'eacute', mb_strtolower($eyecolor->name)))) . '.html';
        $client = new Client();
        ...
        $this->set(compact('eyecolor', 'examples', 'count', 'frequency', 'recent_count', 'recent_frequency', 'age', 'is_labo', 'labo', 'user', 'show_staff'));
    }
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Eyecolors->save($eyecolor)) {
                ...
                $translations = $this->fetchModel('EyecolorsTranslations');
                foreach ($locales as $loc => $value) {
                    if ($loc != $default) {
                        $eyecolorTranslation = $translations->newEmptyEntity();
                        $eyecolorTranslation->id = $eyecolor->id;
                        $eyecolorTranslation->locale = $loc;
                        $eyecolorTranslation->name = $eyecolor->name;
                        $eyecolorTranslation->genotype = $eyecolor->genotype;
                        $eyecolorTranslation->description = $eyecolor->description;
                        $translations->save($eyecolorTranslation);
                    }
                }
