...
class SingularitiesController extends AppController
{
    ...
    public function view($id = null)
    {
        ...
        $age['all'] = $count ? $singularity->roundLifespan([], ['singularity' => $singularity->id]) : __('N/A');
        ...
        $labo = 'http://laborats.weebly.com/' . h(preg_replace('/\s+/', '-', str_replace('é', 'eacute', mb_strtolower($singularity->name)))) . '.html';
        $client = new Client();
        ...
        $this->set(compact('singularity', 'examples', 'count', 'frequency', 'recent_count', 'recent_frequency', 'age', 'is_labo', 'labo', 'user', 'show_staff'));
    }
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Singularities->save($singularity)) {
                ...
                $translations = $this->fetchModel('SingularitiesTranslations');
                foreach ($locales as $loc => $value) {
                    if ($loc != $default) {
                        $singularityTranslation = $translations->newEmptyEntity();
                        $singularityTranslation->id = $singularity->id;
                        $singularityTranslation->locale = $loc;
                        $singularityTranslation->name = $singularity->name;
                        $singularityTranslation->genotype = $singularity->genotype;
                        $singularityTranslation->description = $singularity->description;
                        $translations->save($singularityTranslation);
                    }
                }
