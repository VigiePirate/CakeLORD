...
function setup() {
  ...
  var zoom = d3.behavior.zoom()
    ...
    .translate([window.screen.width/3, 375]); //.translate([380, 330]);

  var svg = d3.select("#familytree").append("svg")
    .attr("viewBox", "0 0 " + window.screen.width + ' ' + window.screen.height ) //.attr("viewBox", "0 0 1000 750" ) //.attr("viewBox", "0 0 1000 660" )
    .attr("preserveAspectRatio", "xMidYMid meet")
    .call(zoom)
    .append('g')
    // Left padding of tree so that the whole root node is on the screen.
    // TODO: find a better way
    .attr("transform", "translate(" + window.screen.width/3 + ",375)"); //.attr("transform", "translate(380,330)");
